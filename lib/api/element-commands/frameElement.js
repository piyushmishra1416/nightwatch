const ProtocolAction = require('/Users/piyush/nightwatch/lib/api/protocol/_base-action.js');
...
const findElement = function(selector) {
  return new Promise((resolve, reject) => {
    ...
    } else { // If selector is an element
      resolve(selector);
    }
  });
};

module.exports = class FrameElement extends ProtocolAction {
  async command(frameId, callback) {
    ...
    if (typeof frameId == 'string') {
      frameId = await findElement.call(this, frameId)
        ...
        .catch(err => {
          throw new Error(`Unable to locate frame  ${frameId}.`);
        });
